...
export default async (req) => {
  ...
  if (req.method === 'POST') {
    ...
    if (t.akce === 'profil' || t.akce === 'podpis') {
      ...
      if (cil === ADMIN_EMAIL && relace.email !== ADMIN_EMAIL)
        return json({ ok: false, chyba: 'Profil a podpis hlavního administrátora mění jen on sám.' }, 403);
    ...
    if (t.akce === 'zaloz') {
      ...
      /* Whitelist rolí musí zůstat i u zakládání (nález při slučování dávek
       * 23. 8. 2026 — patch 3. dávky ho při přepisu větve `zaloz` vypustil).
       * Bez něj by vznikl účet s rolí, kterou matice práv nezná: nikam by se
       * nedostal, ale ani by nešel opravit jinak než ručně v úložišti. */
      ...
      /* Vlastní účet si správce nearchivuje (audit 23. 8. 2026, B31) — archiv
       * nastaví aktivni=false, takže by se sám zamkl (jako B15 u vypnutí). */
      if (email === relace.email && t.archiv)
